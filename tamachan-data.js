// tamachan-data.js : たまフィットPFCアプリ用 AIボット仕様書（マニアック柔軟会話 ＋ チェーン店キルスイッチ ＋ アルコール致死量防止版）
...
- 【お酒のアルコール量(A)の計算式】※超重要※ お酒の量(ml)をそのままアルコール量(g)にするのは【絶対禁止】です！必ず「量(ml) × (アルコール度数(%) ÷ 100) × 0.8」でグラム数を計算しなさい。例：ウイスキー(度数40%) 1000mlの場合、1000 × 0.4 × 0.8 = 320g のアルコール(A)となる。
