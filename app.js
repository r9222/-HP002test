...
function mkCat() {
    const d = document.getElementById('cat-btns'); if(typeof DB === 'undefined') return;
...
function ren() {
    ...
    times.forEach(t => {
        ...
        const sec = document.createElement('div'); sec.className = 'tl-sec'; let aStr = (TG.alcMode && tA>0) ? ` <span style="color:var(--alc)">A${tA.toFixed(0)}</span>` : "";
        sec.innerHTML = `<div class="tl-head ${t}"><div>${emojis[t]} ${t}</div><div class="tl-stats">${tCal}kcal (P${tP.toFixed(0)} F${tF.toFixed(0)} C${tC.toFixed(0)}${aStr})</div></div><ul class="f-list">${items.map(x => {
            let aTag = (TG.alcMode && x.A > 0) ? ` <span style="color:var(--alc)">A${x.A.toFixed(1)}</span>` : ""; let isAlcClass = (x.A > 0) ? "alc" : "";
...
function upd() {
    ...
    if (TG.alcMode) { let elA = document.getElementById('bar-a'); let tboxA = document.getElementById('bar-text-a'); let txA = document.getElementById('rem-a'); if(elA) elA.style.width = Math.min((t.A / 50) * 100, 100) + '%'; if(tboxA) tboxA.textContent = `${t.A.toFixed(1)}g`; }
...
function drawGraph(type, btn) {
    document.querySelectorAll('.g-btn').forEach(b => b.classList.remove('act')); if(btn) btn.classList.add('act'); const box = document.getElementById('chart-box'); box.innerHTML = ''; let data = []; const today = new Date();
...
function drawBodyGraph(mode, btn) {
    document.querySelectorAll('.b-tog-btn').forEach(b => b.classList.remove('act')); if(btn) btn.classList.add('act'); const box = document.getElementById('body-chart-area'); box.innerHTML = ''; const legend = document.getElementById('body-legend'); legend.innerHTML = ''; 
...
function toggleChat() { const win = document.getElementById('tama-chat-window'); const btn = document.getElementById('tama-chat-btn'); if (win.style.display === 'flex') { win.style.display = 'none'; btn.style.display = 'flex'; } else { win.style.display = 'flex'; btn.style.display = 'none'; } }
...
function startGlobalMic() { const win = document.getElementById('tama-chat-window'); const btn = document.getElementById('tama-chat-btn'); if (win.style.display !== 'flex') { win.style.display = 'flex'; btn.style.display = 'none'; } setTimeout(() => { if (!isRecording) toggleMic(); }, 100); }
